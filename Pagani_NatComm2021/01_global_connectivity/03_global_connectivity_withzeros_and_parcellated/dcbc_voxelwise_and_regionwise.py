"""
mp: this code should be able to run global connectivity also in case of 0 voxels within the mask

Compute weighted global brain connectivity (wGBC) on a 4D rsfMRI time-series data.
...
python wdc.py -i $imgfile -m $maskfile -o $outfile
...
outfile=~/data/parc_wgbc01.nii.gz
python wdc.py -i $imgfile -m $maskfile -a $atlasfile -o $outfile
"""
...
def parse_args():
    ...
    """
    parser=OptionParser()
    parser.add_option('-i',"--input",dest='imgfile',help="Filename of the input rsfMRI 4D time-series dataset",default=None)
    parser.add_option('-o',"--output",dest='outname',help="Output filename",default=None)
    parser.add_option('-m',"--mask",dest='maskfile',help="Filename of brain mask",default=None)
    parser.add_option('-a',"--atlas",dest='atlasfile',help="Filename of atlas file to use if parcellating data",default=None)
    parser.add_option('-r',"--radius",dest='radius',help="""Radius. If not provided, the script computes global brain
    ...
    are further away than the given radius.""",default=None)
