...
two_sample=$PWD/07a_t-test_and_cohens_d_all_sites/
...
mkdir $two_sample
    ...
    3dcalc \
        -a $two_sample/stats_${groupA}_vs_${groupB}_group_Tstat.nii.gz \
	-expr "(2*a/sqrt($df))" \
        -prefix $two_sample/stats_${groupA}_vs_${groupB}_group_cohens_d.nii.gz
