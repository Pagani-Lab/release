...
def neuroCombat(dat,
    ...
    np.savetxt(output_name+'.csv',bayes_data.transpose(), delimiter=',')
...
def standardize_across_features(X, design, info_dict):
    ...
    var_pooled[var_pooled==0] = np.median(var_pooled!=0)   #MP
...
def fit_LS_model_and_find_priors(s_data, design, info_dict, mean_only):
    ...
    delta_hat = list(map(convert_zeroes, delta_hat)) #MP
...
output_name_cov = 'subjects_voxels_matrix_combat_with_preserved_covariates'
output_name = 'subjects_voxels_matrix_combat_without_preserved_covariates'

dat = pd.read_csv('voxels_subjects_matrix.csv')
covars = pd.read_csv('abide_discovery_dataset_covariates_for_combat.csv')

print(dat.shape)
print(covars.shape)

# ComBat steps
discrete_cols = ['diagnosis' ]
continuous_cols = ['age_at_scan']
batch_col = ['site_id']

#print("Computing with covariates:")
#data_combat = neuroCombat(dat=dat, covars=covars,categorical_cols=discrete_cols,continuous_cols=continuous_cols, batch_col=batch_col, output_name=output_name_cov)

print("Computing without covariates:")
data_combat = neuroCombat(dat=dat, covars=covars, batch_col=batch_col,output_name=output_name)
